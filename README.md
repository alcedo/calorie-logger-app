...
Pick a **provider and model** on the Today page or the AI page. Claude Code login is the default on a machine that can run the CLI; ChatGPT/Codex is next. An OpenAI API key is a paid opt-in. Local auto never selects it. On Vercel, auto uses a pasted Claude setup-token or a ChatGPT login over HTTP, then `OPENAI_API_KEY`.
...
Vercel Functions cannot write under the app directory, so the host stores SQLite in the instance tmp directory (`calorie-logger.db`). That file is ephemeral. A cold start starts from an empty seeded database.

Vercel Functions cannot spawn `claude` or `codex`. They can still use your subscription over HTTP. Paste a Claude `setup-token` on the AI page, or connect ChatGPT with the device login. Set `CLAUDE_CODE_OAUTH_TOKEN` or `CODEX_AUTH_JSON` on the Vercel project if you want the credential to survive a cold start. `OPENAI_API_KEY` remains a paid fallback. Without any of those, only the ~110 built-in foods resolve.

Open **AI** to connect a subscription. On a machine with the CLIs, Claude shows a login link plus a code to paste. On Vercel, paste a `claude setup-token` instead. ChatGPT uses device-auth on both hosts.

The Claude Code and Codex CLIs must be installed **on the computer that runs this app** when you want the local CLI transport. `scripts/install-ai-clis.sh` does that. If a CLI is missing on that machine, Connect Claude is disabled. ChatGPT connect on Vercel does not need the CLI.
...
Set `AI_PROVIDER=openai` **and** `OPENAI_API_KEY`. Local auto never chooses this provider. On Vercel (`VERCEL=1`), auto uses the key only after a Claude setup-token and a ChatGPT login are both missing.
...
| `CLAUDE_CODE_OAUTH_TOKEN` | Claude setup-token. Prefer `claude auth login` locally. Required on Vercel unless you paste the token in the UI. |
| `CODEX_AUTH_JSON` | ChatGPT OAuth JSON (`accessToken`, `refreshToken`, optional `accountId`). Prefer Connect ChatGPT. |
| `OPENAI_API_KEY` | Paid opt-in. Vercel fallback after subscription tokens. Local auto never selects it. |
